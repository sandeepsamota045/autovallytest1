...
const getStatusMessage = (lang, msg) => {
  if (lang === "ar") {
    return (
      '<!DOCTYPE html><html dir="rtl" lang="ar"><head></head><body><p>' +
      msg +
      "</p></body></html>"
    );
  }
    ...
    .then((user) => {
      ...
      token
        ...
        .catch((err) => {
          ...
          res
            .status(400)
            .send(getStatusMessage(user.language, strings.DB_ERROR + err));
        });
    })
    .catch((err) => {
      console.error(strings.DB_ERROR, err);
...
export const validateEmail = async (req, res) => {
  try {
    ...
    console.error(
      "[user.validateEmail] " + strings.DB_ERROR + " " + req.body.email,
      err
    );
    return res.status(400).send(strings.DB_ERROR + err);
  }
      ...
      // if token is found then check valid user
        ...
        // verify user
          ...
          user.save((err) => {
            ...
            if (err) {
              console.error(
                "[user.confirmEmail] " + strings.DB_ERROR + " " + req.params,
                err
              );
              return res
                .status(500)
                .send(getStatusMessage(user.language, err.message));
            }
            // account successfully verified
              ...
              return res
                ...
                .send(
                  getStatusMessage(
                    user.language,
                    strings.ACCOUNT_ACTIVATION_SUCCESS
                  )
                );
            }
          });
    ...
    // send verification link
      ...
      token.save((err) => {
        ...
        transporter.sendMail(mailOptions, (err, info) => {
          if (err) {
            console.error(
              "[user.resendLink] " + strings.SMTP_ERROR,
              req.params
            );
            return res
              .status(500)
              .send(
                getStatusMessage(
                  user.language,
                  strings.ACCOUNT_ACTIVATION_TECHNICAL_ISSUE +
                    " " +
                    err.response
                )
              );
...
export const updateEmailNotifications = (req, res) => {
  User.findById(req.body._id)
    .then((user) => {
      if (!user) {
        console.error(
          "[user.updateEmailNotifications] User not found:",
          req.body.email
        );
        res.sendStatus(204);
      } else {
