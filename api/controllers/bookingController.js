...
export const deleteBookings = async (req, res) => {
  try {
    ...
    Booking.deleteMany({ _id: { $in: ids } }, async (err, response) => {
      ...
      const additionalDivers = bookings.map((booking) =>
        mongoose.Types.ObjectId(booking._additionalDriver)
      );
...
export const getBooking = (req, res) => {
  Booking.findById(req.params.id)
    ...
    .catch((err) => {
      console.error(
        `[booking.getBooking] ${strings.DB_ERROR} ${req.params.id}`,
        err
      );
      res.status(400).send(strings.DB_ERROR + err);
    });
...
export const getBookings = async (req, res) => {
  try {
    ...
    const companies = req.body.companies.map((id) =>
    ...
    if (user)
      $match.$and.push({
        "driver._id": { $eq: mongoose.Types.ObjectId(user) },
      });
    if (car)
      $match.$and.push({ "car._id": { $eq: mongoose.Types.ObjectId(car) } });
    if (from) $match.$and.push({ from: { $gte: from } }); // $from > from
    if (to) $match.$and.push({ to: { $lte: to } }); // $to < to
    if (pickupLocation)
      $match.$and.push({
        "pickupLocation._id": { $eq: mongoose.Types.ObjectId(pickupLocation) },
      });
    if (dropOffLocation)
      $match.$and.push({
        "dropOffLocation._id": {
          $eq: mongoose.Types.ObjectId(dropOffLocation),
        },
      });
    if (keyword) {
...
export const hasBookings = (req, res) => {
  Booking.find({ driver: mongoose.Types.ObjectId(req.params.driver) })
    ...
    .catch((err) => {
      console.error(
        `[booking.hasBookings] ${strings.DB_ERROR} ${req.params.driver}`,
        err
      );
      res.status(400).send(strings.DB_ERROR + err);
    });
...
export const bookingsMinDate = (req, res) => {
  Booking.findOne(
    { driver: mongoose.Types.ObjectId(req.params.driver) },
    { from: 1 }
  )
    .sort({ from: 1 })
    .then((booking) => {
    ...
    .catch((err) => {
      console.error(
        `[booking.bookingsMinDate] ${strings.DB_ERROR} ${req.params.driver}`,
        err
      );
      return res.status(400).send(strings.DB_ERROR + err);
    });
...
export const bookingsMaxDate = (req, res) => {
  Booking.findOne(
    ...
    .catch((err) => {
      console.error(
        `[booking.bookingsMaxDate] ${strings.DB_ERROR} ${req.params.driver}`,
        err
      );
      return res.status(400).send(strings.DB_ERROR + err);
    });
...
export const cancelBooking = (req, res) => {
  Booking.findOne({ _id: mongoose.Types.ObjectId(req.params.id) })
    ...
    .catch((err) => {
      console.error(
        `[booking.cancelBooking] ${strings.DB_ERROR} ${req.params.id}`,
        err
      );
      return res.status(400).send(strings.DB_ERROR + err);
    });
